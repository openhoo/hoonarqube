...
impl<'a> GraphqlResolver<'a> {
    ...
    pub(crate) fn django_model_str_facts(&self, at: TextSize) -> (bool, bool) {
        let Some(symbol) = self.current.symbols.iter().position(
            |symbol| matches!(symbol, SymbolFact::Class { at: start, .. } if *start == at),
        ) else {
            return (false, false);
        };
        let mut visited = Vec::new();
        (
            self.django_class_property(
                self.current,
                symbol,
                DjangoClassProperty::Model,
                &mut visited,
            ),
            self.django_class_property(
                self.current,
                symbol,
                DjangoClassProperty::StrMethod,
                &mut visited,
            ),
        )
    }
    ...
    fn django_class_property(
        ...
        if property == DjangoClassProperty::StrMethod
            && let Some(method) = str_method
        {
            return *method;
        }
    ...
    fn django_reference_property(
        ...
        match reference {
            RefExpr::Name(name) => {
                ...
                let Some(binding) = module.scopes[scope]
                    .bindings
                    .iter()
                    .rev()
                    .find(|binding| binding.name == *name && binding.at <= at)
                ...
                let marker = (
                    format!("\0django:{}:{scope}:{name}", module.name),
                    u32::from(binding.at) as usize,
                );
                if visited.contains(&marker) {
                    return false;
                }
                visited.push(marker);
                let result = self.django_value_property(
                    module,
                    scope,
                    binding.at,
                    &binding.value,
            ...
            RefExpr::Attribute(base, member) => {
                let Some(name) = self.resolve_module_reference(module, scope, at, base, visited)
                else {
                    return false;
                };
    ...
    fn django_member_property(
        ...
        let marker = (
            format!("\0django-member:{module_name}:{member}"),
            usize::MAX,
        );
        if visited.contains(&marker) {
            return false;
    ...
    fn django_value_property(
        ...
        match value {
            ValueFact::Symbol(symbol) => {
                self.django_class_property(module, *symbol, property, visited)
            }
            ValueFact::Reference(reference) => self.django_reference_property(
                module,
                scope,
                TextSize::new(u32::from(at).saturating_sub(1)),
                reference,
                property,
                visited,
            ),

...
/// `False`); both Flask and Django expose this exact API shape. Sonar
/// treats any present flag kwarg as compliant — the value need not be a
/// literal True.
pub(crate) fn cookie_flag_missing(call: &ruff_python_ast::ExprCall, flag: &str) -> bool {
    is_call_method(call, "set_cookie") && keyword_value(&call.arguments, flag).is_none()
}

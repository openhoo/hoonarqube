...
/// python:S9073 — a test assertion joining independent facts: an `and`
/// chain hides which operand failed, and `assert not (a or b)` is its
/// De Morgan equivalent. Plain `assert a or b` stays silent because
/// splitting it would change the meaning from "at least one holds" to
/// "all hold"; the same exclusion covers a top-level `or` with a
/// nested `and`. The assert statement anchors the finding.

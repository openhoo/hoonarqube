...
// value is a non-empty object literal is reported. Identifier parameters are
// anchored on the identifier and name the parameter in the message;
// destructuring parameters are anchored on the object literal. No auto-fix
// is offered: a shared constant would change the fresh mutable-allocation
// semantics that the zod v4 `ProcessParams` defaults rely on.
//
...
";
        ...
        assert_eq!(issue.range.start.line, 7);
        assert!(

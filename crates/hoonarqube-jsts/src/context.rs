/// Catalog-backed parameters for the batch rules. Defaults are the frozen
/// catalog values: `S104` `maximum=1000`, `S138` `max=200`, `S1451`
...
impl From<&AnalyzerOptions> for RuleOptions {
    fn from(options: &AnalyzerOptions) -> Self {
        Self {
            maximum_lines_of_code: options.maximum_lines_of_code,
            maximum_function_lines: options.maximum_function_lines,
            header_format: options.header_format.clone(),
            header_is_regular_expression: options.header_is_regular_expression,
            comment_pattern: options.comment_pattern.clone(),
            password_words: options.password_words.clone(),
            secret_entropy_sensibility: 5.0,
            secret_words: options.secret_words.clone(),
            format_functions: options.format_functions.clone(),
            format_classes: options.format_classes.clone(),
            format_variables: options.format_variables.clone(),
            duplicate_string_threshold: options.duplicate_string_threshold,
            ignored_strings: options.ignored_strings.clone(),
            single_quotes: options.single_quotes,
            jsx_attribute_whitelist: options.jsx_attribute_whitelist.clone(),
        }
    }
}

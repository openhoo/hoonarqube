...
use crate::rules::run_migrated;
use crate::rules::expression::s1528_constructor_calls::{argument_expression};
use crate::rules::expression::walker::{call_property, is_equality_operator, regex_pattern_text};
use crate::rules::one_stmt::s122_suite::{check_suite};
use crate::support::{IssueSink, LineIndex, RuleScope, ScannedComment, assignment_target_name, binding_identifier_name, callee_name, constructor_name, expression_root_name, extension_of, file_metrics, identifier_name, is_identifier_byte, member_object, member_root_name, member_rooted_at, next_non_trivia_offset, previous_non_trivia_offset, property_key_name, scan_comments, sort_issues, source_slice, source_type_for, span_issue, span_text, span_text_contains, statement_as_expression, static_property_name, to_u32, unparenthesized, update_target_name};
use crate::engine::pattern_parser::{AnchorKind, ClassItem, GraphemeComponentKind, GroupReference, ParsedRegex, PatternNode, RegexSite, constructor_regex_site, contains_unbounded_quantifier, grapheme_component_kind, literal_string_value, node_can_match_empty, parse_regex_pattern, pattern_complexity, regex_flags_text, regex_literal_argument, regex_site_from_literal, sequence_can_match_empty, walk_pattern_nodes};
use crate::engine::scope_model::{FunctionCensus, FunctionParamMapCollector, LiteralKind, TbFlow, TbHalt, TbKind, TbModel, build_tb_model, callee_member_name, collect_array_binding_names, function_body_span, function_parameters, kind_is_composite, kind_is_numeric, literal_kind, member_optional, parameter_names, tb_assignment_target};
...
fn analyze_with_rules(
    ...
    issues.extend(rules::s103_line_length::check(&ctx));
    issues.extend(rules::s105_tab_characters::check(&ctx));
    issues.extend(rules::s113_newline_at_eof::check(&ctx));
    issues.extend(rules::s1131_trailing_whitespace::check(&ctx));
    issues.extend(rules::s104_lines_of_code::check(&ctx));
    issues.extend(rules::s1451_file_header::check(&ctx));
    issues.extend(rules::s1134_s1135_task_tags::check(&ctx));
    issues.extend(rules::s1291_nosonar::check(&ctx));
    issues.extend(rules::s139_disallowed_comment_pattern::check(&ctx));
    issues.extend(rules::s125_commented_out_code::check(&ctx));
    ...
    issues.extend(rules::one_stmt::run_all(&ctx));
    issues.extend(rules::statement::run_all(&ctx));
    issues.extend(rules::expression::run_all(&ctx));
    issues.extend(rules::binding::run_all(&ctx));
    issues.extend(rules::func_len::run_all(&ctx));
    issues.extend(rules::eval::run_all(&ctx));
    issues.extend(rules::naming::run_all(&ctx));
    issues.extend(rules::duplicate::run_all(&ctx));

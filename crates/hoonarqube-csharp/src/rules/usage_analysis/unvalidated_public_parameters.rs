...
pub(crate) fn check(root: Node<'_>, source: &str, language: CsLanguage) -> Vec<Issue> {
    ...
    for method in collect_kinds(root, &["method_declaration"]) {
        ...
        if let Some(body) = body_of(method) {
            check_parameters(method, body, source, language, &mut issues);
        }
    }
    issues
}
...
fn nullable_parameter_name<'a>(parameter: Node<'_>, source: &'a str) -> Option<&'a str> {
    ...
    collect_kinds(body, &["identifier"])
        ...
        .find(|identifier| {
            !is_error_tainted(*identifier)
                ...
                && identifier
                    .parent()
                    .is_some_and(|parent| is_dereference_parent(parent, *identifier))
        })
}

fn is_dereference_parent(parent: Node<'_>, identifier: Node<'_>) -> bool {
    matches!(
        parent.kind(),
        "member_access_expression" | "element_access_expression" | "element_binding_expression"
    ) && first_named_child(parent).is_some_and(|base| base.id() == identifier.id())

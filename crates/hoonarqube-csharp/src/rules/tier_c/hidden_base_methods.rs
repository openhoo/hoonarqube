...
use crate::project_index::{IndexedParameter, ProjectTypeIndex};
...
/// csharpsquid:S4019 — a derived method hides a same-name base method when
/// their signatures are compatible: same arity, equal ref-kind, and every
/// derived parameter type equal to or wider than the base's, so calls
/// intended for the base method can no longer bind to it. Base resolution
/// uses the file-local type table, and every accepted project file when a
/// [`ProjectTypeIndex`] is supplied.
pub(crate) fn check(
    root: Node<'_>,
    source: &str,
    language: CsLanguage,
    ...
    for declaration in local_type_declarations(root) {
        ...
        for (name_node, method_name, derived_parameters) in derived_methods(declaration, source) {
            for hidden in hidden_sites(base_name, method_name, &types, source, project) {
...
/// when present, otherwise from the file-local base declaration.
fn hidden_sites<'a>(
    base_name: &'a str,
    method_name: &str,

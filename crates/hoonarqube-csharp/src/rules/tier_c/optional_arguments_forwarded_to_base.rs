...
/// csharpsquid:S3466 — an override forwarding to `base.` must pass its
/// caller-supplied optional parameter rather than silently use the base
...
pub(crate) fn check(root: Node<'_>, source: &str, language: CsLanguage) -> Vec<Issue> {
    collect_kinds(root, &["invocation_expression"])
        ...
        .filter_map(|call| {
            ...
            let parameters = parameters_of(enclosing);
            let defaulted: std::collections::HashSet<&str> = parameters
                .iter()
                .copied()
                ...
                .collect();
            (!defaulted.is_empty()).then_some((call, parameters.len()))
        })
        .filter(|(call, parameter_count)| invocation_arguments(*call).len() < *parameter_count)
        .map(|(call, _)| {
            issue(
                language,
                "S3466",
                "Pass the missing user-supplied parameter value to this 'base' call.",

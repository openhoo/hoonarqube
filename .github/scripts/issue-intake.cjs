...
const NOTICE_MARKER_RE = /<!--\s*hoonarqube-issue-intake:v1\s+needs-info-owned=(true|false)\s*-->/i;
...
function buildNotice(validation, options = {}) {
  ...
  const marker = `<!-- hoonarqube-issue-intake:v1 needs-info-owned=${needsInfoOwned ? 'true' : 'false'} -->`;
  const problems = [];
  const seen = new Set();
  ...
  return [
    ...
    '',
    'Do not post suspected vulnerability details here; use [private vulnerability reporting](https://github.com/openhoo/hoonarqube/security/advisories/new). This automated check does not verify or close the report.',
  ].join('\n');
}

...
def generate_solution(
    ...
    invalid_rules = [rule for rule in rule_ids if not rule.startswith("S")]
    if invalid_rules:
    ...
    test_sources = {
        ...
        if source.stem.split("_", 1)[0].upper() in TEST_RULE_IDS
    }
    if test_sources:
        test_framework_dir = projects_dir / "test-framework"
        test_framework_dir.mkdir()
        (test_framework_dir / "TestFramework.cs").write_text(TEST_FRAMEWORK_SOURCE)
        (test_framework_dir / "TestFramework.csproj").write_text(
...
"""
    ...
    nfluent_sources = {
        source
        for source in sources
        if source.stem.split("_", 1)[0].upper() == "S2970"
    }
    if nfluent_sources:
        nfluent_dir = projects_dir / "nfluent-framework"
        nfluent_dir.mkdir()
        (nfluent_dir / "NFluent.cs").write_text(NFLUENT_FRAMEWORK_SOURCE)
        (nfluent_dir / "NFluent.csproj").write_text(
            """<Project Sdk=\"Microsoft.NET.Sdk\">
  <PropertyGroup>
    <TargetFramework>net10.0</TargetFramework>
  ...
  <ItemGroup>
    <Compile Include=\"NFluent.cs\" />
  </ItemGroup>
</Project>
"""
        )
    azure_function_sources = {
        source
        for source in sources
        if source.stem.split("_", 1)[0].upper()
        in {"S6419", "S6420", "S6421", "S6422", "S6423"}
    }
    if azure_function_sources:
        azure_dir = projects_dir / "azure-functions-framework"
        azure_dir.mkdir()
        (azure_dir / "AzureFunctions.cs").write_text(AZURE_FUNCTIONS_SOURCE)
        (azure_dir / "AzureFunctions.csproj").write_text(
            """<Project Sdk=\"Microsoft.NET.Sdk\">
  <PropertyGroup>
    <TargetFramework>net10.0</TargetFramework>
  ...
  <ItemGroup>
    <Compile Include=\"AzureFunctions.cs\" />
  </ItemGroup>
</Project>
"""
        )
    durable_sources = {
        source
        for source in sources
        if source.stem.split("_", 1)[0].upper() == "S6424"
    }
    if durable_sources:
        durable_dir = projects_dir / "durable-task-framework"
        durable_dir.mkdir()
        (durable_dir / "DurableTask.cs").write_text(DURABLE_TASK_SOURCE)
        (durable_dir / "DurableTask.csproj").write_text(
            """<Project Sdk=\"Microsoft.NET.Sdk\">
  <PropertyGroup>
    <TargetFramework>net10.0</TargetFramework>
  ...
  <ItemGroup>
    <Compile Include=\"DurableTask.cs\" />
  </ItemGroup>
</Project>
"""
        )
    ...
    for ordinal, source in enumerate(sources):
        ...
        if source in nfluent_sources:
            project_references += (
                '\n    <ProjectReference Include="../nfluent-framework/NFluent.csproj" />'
            )
        if source in azure_function_sources:
            project_references += (
                '\n    <ProjectReference Include="../azure-functions-framework/AzureFunctions.csproj" />'
            )
        if source in durable_sources:
            project_references += (
                '\n    <ProjectReference Include="../durable-task-framework/DurableTask.csproj" />'
            )
  ...
  <ItemGroup>{analyzer_items}{project_references}

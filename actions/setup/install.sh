...
if [[ "$RUNNER_OS_VALUE" != Linux || "$RUNNER_ARCH_VALUE" != X64 ]]; then
  echo "::error::Hoonarqube v${version} publishes binaries only for Linux X64 runners."
  exit 2
fi
...
download_dir="$(mktemp -d "${RUNNER_TEMP}/hoonarqube-download.XXXXXXXX")"
...
extract_dir="$(mktemp -d "${RUNNER_TEMP}/hoonarqube-extract.XXXXXXXX")"
tar -xzf "$archive" -C "$extract_dir"
source_binary="${extract_dir}/${stem}/hoonarqube"
if [[ ! -f "$source_binary" ]]; then
  echo "::error::Archive ${archive_name} contains no hoonarqube binary at the expected path."
  exit 1
fi

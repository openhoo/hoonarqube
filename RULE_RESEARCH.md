...
| [gosec G124](https://github.com/securego/gosec/blob/master/RULES.md) | `hoonarqube-go:G124` | Exact `net/http.Cookie` keyed literals; reports omitted or provably disabled `Secure`, `HttpOnly`, or `SameSite`. Positional and dynamically configured values stay silent. |
...
| [Ruff S113](https://docs.astral.sh/ruff/rules/request-without-timeout/) | `hoonarqube-python:request-without-timeout` | Import and shadow resolution restricts findings to `requests`; explicit finite timeout or unknown `**kwargs` stays silent. `httpx` is deliberately excluded because it has a default timeout. |
...
| [Clippy `suspicious_open_options`](https://rust-lang.github.io/rust-clippy/master/index.html#suspicious_open_options) | `hoonarqube-rust:suspicious-open-options` | Standard `OpenOptions` provenance and complete inline builder state prove `create(true)` without explicit truncation or exclusive creation. |

...
# Hoonarqube issue triage

This repository-tracked skill is an independent Hoonarqube workflow. It is
...
global installation. The repository's analyzer contracts, evidence formats,
...
idempotent comment marked `<!-- hoonarqube:triage-brief:v1 -->` beginning with
...
| --- | --- | --- |
| `Bug` | `bug` | evidence-based `false-positive`, `false-negative`, `regression`, `crash`, or `unsafe-fix` |
...
under a comparable setup. `crash` and `unsafe-fix` require observed evidence,
not severity guesses.

Optional labels are evidence-driven and may include the exact language
(`language:python`, `language:javascript`, `language:typescript`,
`language:csharp`, `language:go`, `language:rust`, `language:java`, or
`language:ruby`), area (`area:cli`, `area:exports`, `area:metrics`, or
`area:ci`), and priority (`priority:critical`, `priority:high`,
...
### 3. Use authoritative analyzer evidence

For a detector or coverage claim, use an actual executable registry and a
bounded profile run. In particular, `catalog/github-code-quality.json` is a
definitions-only catalog: its rows and `implementation_status` do not prove
runtime coverage. Check the language-specific `GITHUB_QUALITY_RULE_IDS`, the
`github_quality` dispatch, the rule registry, and an actual
`analyze --profile github-code-quality --format sarif` result. The catalog,
profile, and native catalog are separate contracts.
...
- **Coverage request:** a desired detector or supported behavior is not in the
  executable profile/registry, and no existing contract promises it. Add
  `coverage` after checking the actual registry, not merely a definitions file.
...
- exact source commit, scope, profile, runtime, range, and completeness limits
  are recorded for analyzer claims;
- CodeQL metadata was not mistaken for executable coverage;

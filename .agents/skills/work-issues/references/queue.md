...
```sh
mkdir -p "$RUN_DIR"
gh issue list -R openhoo/hoonarqube --state open --limit 10000 \
...
```sh
gh issue list -R openhoo/hoonarqube --state all --limit 10000 \
  --json number,title,state,labels,createdAt,updatedAt,closedAt > "$RUN_DIR/current-issues.json"
gh pr list -R openhoo/hoonarqube --state all --limit 10000 \
...
python3 .agents/skills/work-issues/scripts/issue_queue.py --db "$RUN_DIR/queue.sqlite" \
  ...
  --surfaces crates/hoonarqube-rust/src/lib.rs
```
...
```json
{
  ...
  "checks": [
    {"command": "cargo test --locked -p hoonarqube-rust exact_regression_name", "exit_code": 0}
  ],
  "acceptance": {
